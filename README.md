...
A channel key is exclusively the native `+k` parameter in `C::<channel>::modes`, for example `+ntk secret`. It protects the first JOIN as well as later joins. `C::<channel>::modes` rejects the registration marker `r`, member ranks (`q`, `a`, `o`, `h`, `v`), and `P`; persistence is expressed only by the `C::options` `PERSISTENT` bit. Native `+r` is UDB-owned registered-channel state: it is applied when the channel is registered even if the channel already exists, removed while `C::<channel>::suspend` exists or the profile is deleted, and restored together with founder `+q` when suspension is lifted. An identified founder receives UDB-owned `+q`. While `C::<channel>::suspend` exists, every local user who joins the channel receives a ChanServ notice carrying the stored record value as the suspension reason.
