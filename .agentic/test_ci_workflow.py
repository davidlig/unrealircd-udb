...
class WorkflowContractTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.run_scripts = "\n".join(
            step.get("run", "") for step in cls.test_job["steps"]
        )
    ...
    def test_normal_and_sanitized_builds_share_one_matrix_job(self):
        ...
        profiles = {
            entry["profile"]: entry
            for entry in self.test_job["strategy"]["matrix"]["include"]
        }
    ...
    def test_unrealircd_uses_supported_quick_config(self):
        invocations = [
            line.strip()
            for line in self.run_scripts.splitlines()
    ...
    def test_runtime_paths_are_portable(self):
        ...
        self.assertIn(
            'UDB_TEST_IRCD_ROOT=$RUNNER_TEMP/unrealircd',
            self.run_scripts,
        )
